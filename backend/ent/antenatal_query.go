...
type AntenatalQuery struct {
	...
	withBabystatus *BabystatusQuery
	withFKs        bool
...
func (aq *AntenatalQuery) QueryUser() *UserQuery {
...
func (aq *AntenatalQuery) QueryPatient() *PatientQuery {
...
// QueryBabystatus chains the current query on the babystatus edge.
func (aq *AntenatalQuery) QueryBabystatus() *BabystatusQuery {
...
func (aq *AntenatalQuery) WithUser(opts ...func(*UserQuery)) *AntenatalQuery {
	query := &UserQuery{config: aq.config}
...
func (aq *AntenatalQuery) WithPatient(opts ...func(*PatientQuery)) *AntenatalQuery {
	query := &PatientQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//  WithBabystatus tells the query-builder to eager-loads the nodes that are connected to
// the "babystatus" edge. The optional arguments used to configure the query builder of the edge.
func (aq *AntenatalQuery) WithBabystatus(opts ...func(*BabystatusQuery)) *AntenatalQuery {
	query := &BabystatusQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withBabystatus = query
	return aq
}
...
func (aq *AntenatalQuery) sqlAll(ctx context.Context) ([]*Antenatal, error) {
	var (
		...
		loadedTypes = [3]bool{
			aq.withUser != nil,
			aq.withPatient != nil,
			aq.withBabystatus != nil,
		}
	)
	if aq.withUser != nil || aq.withPatient != nil || aq.withBabystatus != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withBabystatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Antenatal)
		for i := range nodes {
			if fk := nodes[i].babystatus_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "babystatus_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Babystatus = n
			}
		}
	}
