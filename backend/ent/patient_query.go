...
func (pq *PatientQuery) QueryAntenatals() *AntenatalQuery {
...
func (pq *PatientQuery) WithAntenatals(opts ...func(*AntenatalQuery)) *PatientQuery {
	query := &AntenatalQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withAntenatals = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withAntenatals; query != nil {
		...
		query.Where(predicate.Antenatal(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.AntenatalsColumn, fks...))
		}))
