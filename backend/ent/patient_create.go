...
func (pc *PatientCreate) AddAntenatalIDs(ids ...int) *PatientCreate {
	pc.mutation.AddAntenatalIDs(ids...)
	return pc
}

// AddAntenatals adds the antenatals edges to Antenatal.
func (pc *PatientCreate) AddAntenatals(a ...*Antenatal) *PatientCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return pc.AddAntenatalIDs(ids...)
}
...
func (pc *PatientCreate) createSpec() (*Patient, *sqlgraph.CreateSpec) {
	...
	if nodes := pc.mutation.AntenatalsIDs(); len(nodes) > 0 {
